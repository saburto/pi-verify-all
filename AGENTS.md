...
- **Auto-retry**: after a failure, sets `pendingReRun = true`. On `agent_end`, if the flag is set, re-runs the pipeline automatically (up to `maxRetries` from config, default 5). When retries are exhausted, runs `onExhausted` command (if configured) before sending the final failure message. Configurable via `maxRetries` and `onExhausted` fields in `.pi/verify.json`.
